...
class ThermalPrinter {
  constructor(initConfig) {
    ...
    if (initConfig.type === this.types.STAR) {
      ...
      this.append(Buffer.from([0x1b, 0x33, 10]));
    }
  ...
  splitTextOnSpace(text, columns) {
    let currentSpace = columns
    let prevSpace = null
    for (let i = 0; i < text.length; i++) {
      if (text[i] === ' ') {
        if (currentSpace) prevSpace = currentSpace
        currentSpace = i
        if (currentSpace >= columns) {
          return [
            text.substring(0, prevSpace),
            text.substring(prevSpace + 1),
          ]
        }
      }
    }
    return [
      text.substring(0, currentSpace),
      text.substring(currentSpace + 1),
    ]
  }
  
  textToLines(text, columns) {
    const lines = []
    while (text.length) {
      if (text.length <= columns) {
        ...
        const split = this.splitTextOnSpace(text, columns)
        lines.push(split[0])
        text = split[1]
      }
    }
  ...
  tableCustom(data, tableConfig) {
    var tableWidth = typeof tableConfig === 'object' && tableConfig.cols ? tableConfig.cols : this.config.width
    var cellWidth = tableWidth / data.length;
    var secondLine = [];
    var secondLineEnabled = false;
    let secondLineText = null
    var padChar = typeof tableConfig === 'object' && tableConfig.padChar ? tableConfig.padChar : ' '
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (obj.width) {
        cellWidth = tableWidth * obj.width;
      } else if (obj.cols) {
        cellWidth = obj.cols
      }
      ...
      if (cellWidth < obj.text.length) {
        ...
        const parts = this.textToLines(obj.text, cellWidth)
        // obj.text = obj.text.substring(0, cellWidth - 1)
        obj.text = parts.shift()
      ...
      if (obj.align == "CENTER") {
        ...
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(padChar));
        }
